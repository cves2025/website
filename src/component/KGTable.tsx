...
interface KGTableProps {
  whichClass: string;
  resultData: StudentResultData | null;
}

function KGTable({ whichClass, resultData }: KGTableProps) {
  const [studentProfileData, setStudentProfileData] = useState<
    StudentProfileRow[]
  >([]);
  const [studentMarksData, setStudentMarksData] = useState<MarksRow[]>([]);

  const getHighlightClass = (subject: string | undefined): string => {
