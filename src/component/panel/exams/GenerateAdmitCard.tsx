    ...
    <div className="max-w-5xl">
      ...
      {selectedExam &&
        cards.map((card) => (
          <div
            key={card.id}
            className="mx-auto mt-2 max-w-3xl overflow-hidden rounded-xl border-4 border-double border-blue-800 bg-white shadow-lg print:break-after-page print:last:break-after-auto"
          >
            {/* Card header */}
            ...
            {/* Subject wise schedule: the written papers table comes first, and
                the practical / viva papers of the same subjects are printed in
                their own table right below it (the schedule itself allows one
                row per subject, so each row can expand into several types). */}
