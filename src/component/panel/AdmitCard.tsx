...
const SUBJECTS: string[] = ["English", "Hindi", "Math", "Science", "Social Science", "Computer"];
...
const sampleStudent: AdmitCardStudent = {
  firstName: "Aarav",
  lastName: "Sharma",
  enrollment: "1001",
  className: "5",
...
function AdmitCard() {
  const [students] = useState<AdmitCardStudent[]>(() =>
    JSON.parse(localStorage.getItem("cves_students") || "[]")
  );
  const [enrollmentQuery, setEnrollmentQuery] = useState("");
  const [card, setCard] = useState<AdmitCardStudent>(sampleStudent);
