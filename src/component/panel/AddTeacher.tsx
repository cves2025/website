...
interface StoredTeacher extends TeacherInput {
  id: number;
}
...
function AddTeacher() {
  const [teacher, setTeacher] = useState<TeacherInput>(emptyTeacher);
  ...
  const handleChange = (
    ...
    setTeacher((prev) => ({ ...prev, [name]: value }) as TeacherInput);
  };
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const list: StoredTeacher[] = JSON.parse(
      localStorage.getItem("cves_teachers") || "[]"
    );
