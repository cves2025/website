...
function BulkStudentImport({ onImported }) {
  ...
  const readFile = async (e) => {
    ...
    try {
      ...
      setError(parseError.message || "Could not read the file.");
      setStudents([]);
      setErrors([]);
      setFileName("");
    } finally {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      console.warn("Firestore import failed, saved locally:", apiError.message);
      setMessage(
        `⚠️ Backend not reachable (${apiError.message}). ${students.length} student(s) saved locally for now. They will need re-importing once the backend is live.`
      );
      onImported?.(merged);
      setLoading(false);
      return;
