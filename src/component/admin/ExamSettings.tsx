...
interface ExamMessageState {
  unitTest1Message: string;
  unitTest2Message: string;
  halfYearlyMessage: string;
  annualMessage: string;
}

interface ExamErrorState {
  unitTest1Error: string;
  unitTest2Error: string;
  halfYearlyError: string;
  annualError: string;
}

const EMPTY_MESSAGES: ExamMessageState = {
  unitTest1Message: "",
  unitTest2Message: "",
  halfYearlyMessage: "",
  annualMessage: "",
...
const EMPTY_ERRORS: ExamErrorState = {
  unitTest1Error: "",
  unitTest2Error: "",
  halfYearlyError: "",
  annualError: "",
...
function ExamSettings() {
  ...
  const [message, setMessage] = useState<ExamMessageState>(EMPTY_MESSAGES);
  // set error for all type exam
  const [error, setError] = useState<ExamErrorState>(EMPTY_ERRORS);

  const setErrorMessageEmpty = () => {
    setError(EMPTY_ERRORS);
    setMessage(EMPTY_MESSAGES);
  };

  const dateAddHandler = async (examType: string, examDate: string) => {
